...
class Sudoku
  ...
  # JS: added this paste_input block to allow direct copy and paste from sudokuweb.org, must end input with an empty line.
  def paste_input
    for i in (0..80)
      this_input = gets.to_i
      @board[i / 9][i % 9] = this_input unless this_input == 0
    end
  end
